...
when I later decide to spend it. This judgment of validity consists of two
...
bank or the payment processor, to maintain a balance and make a judgment on
...
balances and make a judgment on whether a transaction is valid are relied upon
...
These systems will be \emph{self-enforceable}.
...
with the honest parties, each of which will also be modeled as a Turing Machine. For the
...
these are modeled as Turing Machines~\cite{turing} with additional access to a random tape.
...
in her endeavor to break the protocol. The game outputs a boolean output: \emph{true}
...
giving
her
polynomial time in $\kappa$ to run, as well as some additional arguments which may
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.7 \columnwidth,keepaspectratio]{01-setting/figures/game-based-security.pdf}
...
\import{./01-setting/}{algorithms/alg.game.tex}
...
can \emph{guess} such secrets by taking a random guess. If our secret is uniformly sampled from the
...
willing to accept, as it is unavoidable. We call these strings that are highly random
and almost unguessable \emph{high-entropy}\index{High-entropy} strings.
...
Functions in the form of $1 - \negl$ are called \emph{overwhelming}:

\begin{definition}[Overwhelming function]\index{Overwhelming function}
  A function $f(\kappa)$ is \emph{overwhelming} if it is of the form
  $1 - \negl(\kappa)$ for some negligible function $\negl$.
\end{definition}

We would like our protocols to be secure with overwhelming probability.
...
\begin{definition}[Security]
  A protocol $\Pi$ is secure with respect to game $\textsf{Game}$ if

  \[
    \forall PPT \mathcal{A}: \exists \text{negligible function } \negl:
    \Pr[\textsf{Game}_{\Pi,\mathcal{A}}(\kappa) = 1] \leq \negl(\kappa)
  \]
\end{definition}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.3 \columnwidth,keepaspectratio]{01-setting/figures/p-vs-np.pdf}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.4 \columnwidth,keepaspectratio]{01-setting/figures/reduction.pdf}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.5 \columnwidth,keepaspectratio]{01-setting/figures/peer-to-peer-network.pdf}
    \label{fig.network}
...
one side of the network to the other by traveling through intermediaries. This is achieved
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.5 \columnwidth,keepaspectratio]{01-setting/figures/eclipsed-peer-to-peer-network.pdf}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.5 \columnwidth,keepaspectratio]{01-setting/figures/sybil-attack.pdf}
...
The nature of money as a social construct is explored in many books on economics and anthropology.
One fun and accessible book exploring the tendency of humans to make up stories is Harari's
\emph{Sapiens}~\cite{harari-sapiens}. Focusing more on the topic of money and its anthropological history
is Graeber's \emph{Debt}~\cite{graeber-debt}. Both of these books are non-technical.
...
\emph{Modern Cryptography}, and a good way to learn computational reductions. If you're not very
familiar with the language of computation and complexity such as $\textsf{P}$ and $\textsf{NP}$,
this is a great introductory book on the subject!
