...
like Figure~\ref{fig.x-vs-z}. Eve has some successful queries (labeled $Z$ at the bottom), but the density
of these successful queries in time is smaller than the density of the honest parties' successful
queries (labeled $X$ at the top). While Eve can get lucky for a couple of blocks (in the picture, she gets
...
successful queries (labeled $X$ at the top) might be more dense than the adversarially successful queries
(labeled $Z$ at the bottom) due to honest majority ($t < n - t$), it is still possible that
the convergence opportunities (labeled $Y$ in the middle) are fewer than the adversarially successful
...
transactions. In order to get ledger liveness, we hope to occasionally see an honestly
