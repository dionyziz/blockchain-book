...
\begin {lemma}[Chain Growth Lemma]
    Suppose that at round $r$, an honest party $P$ has a chain of length $\ell$. Then by round $r + j \geq r$, every honest party has adopted a chain of length at least $\ell + \sum_{i=r}^{r + j - 1} X_i$.
\end {lemma}

\begin{proof}
We will prove by induction that all parties at round $r + j$ have a chain with at least $l + \sum_{i=r}^{r + j - 1}$ blocks,
for all values of $j \geq 0$.

The induction is on $j$. In the base case $j=0$,
if an honest party has a chain $C$ of length $\ell$ at round $r$, then that party broadcast $C$ at a round earlier than $r$.
It follows that every honest party will receive $C$ by round $r$, and therefore adopts a chain of length at least
$|C| = \ell = \ell + \sum_{i=r}^{r + 0 - 1} X_i$ by the longest chain rule.

For $j>0$.
For the inductive step, suppose that for all honest parties
$P$, $|C^P_{r+j}| \geq \ell + \sum_{i=r}^{r+j-1} X_i$. We are interested in the length
$|C^P_{r+j+1}|$.

Consider the following two cases:
\begin{enumerate}
\item Case 1: $X_{r+j} = 0$. Due to the longest chain rule, $|C^P_{r + j + 1}|$ must be at least as long as $|C^P_{r+j}|$, and $\sum_{i=r}^{r+j} X_i = \sum_{i=r}^{r+j - 1} X_i$. Therefore, $|C^P_{r+j+1}| \geq |C^P_{r+j}| \geq \ell + \sum_{i=r}^{r + j + 1} X_i$. The inequality is due to the fact that the adversary could also have broadcast blocks during $r+j$.
\item Case 2: $X_{r+j} = 1$. By the definition of $X$, some honest party $P$ has mined a block at round $r+j$. At round $r + j$ all parties have adopted chains of length $|C_{r+j}|$, so due to the longest chain rule, $P$ must have mined a chain of at least length $|C_{r+j}| + 1$ at round $r + j$. This block was then broadcast by $P$ at the end of round $r+j$ and, by the longest chain rule, was adopted by all honest parties by round $r+j+1$. Therefore, $|C^P_{r + j + 1}| = |C^P_{r + j}| + 1 \geq \ell + \sum_{i=r}^{r + j} X_i$.
\end{enumerate}
\end{proof}

Since $X_r$ only indicates whether there is at least one successful honest query, we do not overestimate chain length by counting multiple honest parties mining blocks that are forks of each other. This lemma will later enable us to prove Chain Growth when we should that sometimes $X_r = 1$.
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=14cm]{12-earnest2/figures/honest-maj}
...
\begin{itemize}
    ...
    \item The probability of an honestly successful round is $f=\E[X_r]$. Recall that we have calculated that $\E[Y_r] \geq f (1-f)$ in \Cref{eq.y_lower_bound}, so $\E[Y(S)]$ is at most $f$ relative distance away from $\E[X(S)]$. Following similar logic as above, we secure $f$ to the right of $\E[Y(S)]$ and $f$ to the left of $\E[X(S)]$.

    \item Finally, $\E[X(S)]$ is at most $f$ relative distance away from $|S|pq(n-t)$. We get this from \Cref{eq.f_lower_bound}.
...
\begin{figure*}
    \centering
    \vspace*{\fill}
    \includegraphics[width=\linewidth]{12-earnest2/figures/backbone-vars.pdf}
...
\begin{problems}
    ...
    \item Consider \emph{bitcoin backbone executions} with $n = 10$, $t = 2$,
          ...
          the probability of an execution of interest \emph{not} being typical is at most

          \[
            4 \cdot 10^{12} \cdot e^{- \epsilon ^ 2 \cdot \lambda \cdot f / 3}
            + 4 \cdot 10^{20} \cdot 2^{-\kappa}
          \]
          ...
          \begin{enumerate}
            ...
            \item A Chernoff interval parameter $\lambda$ that ensures typical
                  executions occur with probability at least $1 - 2^{-128}$.
    ...
    \item Consider a proof-of-work longest chain protocol with a $1/3$ adversary
        in a population of $n = 3$ nodes with a hash rate of $q = 3$.
        The security parameter is $\kappa = 256$.

        \begin{enumerate}
            \item What is a valid value of the honest advantage $\delta$?
            \item Choose numeric parameters $\epsilon$ and $f$ to ensure
                safety and liveness.
            \item Calculate the exact numeric probability $p$ of a successful
                query.
            \item Calculate a numeric value for the mining target $T$ to
                match the above.
        \end{enumerate}
    ...
    \item When we implemented the Selfish Mining simulation in class using the
          ...
          \item Consider a bitcoin backbone execution in the usual synchronous lockstep model.
                Suppose that, at the beginning of the execution, the genesis block $G$ already
                has two children $B_1$ and $B_2$ forming a fork of length $1$. From this point
                onwards, the honest parties mine exclusively on top of $B_1$ while the adversary
                mines exclusively on top of $B_2$. The adversary withholds every block it
                produces and only intends to release them once it has built a private chain of
                $k$ blocks extending $B_2$.

                We are interested in the following event $E_k$:
                \begin{quote}
                \itshape Consider the rounds $R_H$ and $R_{\mathcal{A}}$ at which the honest
                parties and the adversary mine a chain of $k$ blocks, respectively. The event
                $E_k$ occurs if $R_H \leq R_{\mathcal{A}}$, i.e. the adversary mined $k$
                blocks before or at the same round as the honest parties mined a chain of $k$
                blocks.
                \end{quote}
                \begin{enumerate}
                \item Calculate the probability $\Pr[E_k]$.
                \item Find the limit of $\Pr[\lim_{k \to \infty} E_k = 1]$, assuming honest
                    majority.
                \end{enumerate}

                Note: Honest parties extend their longest chain by one block every successful
                round, while the adversary just need $k$ succesful queries in order to mine $k$
                blocks.


    \item Answer the following questions in the Backbone model with static difficulty.

        \begin{enumerate}
        \item Consider executions with parameters
        ($n, q, t, f, \epsilon, T, \mu, \ell, s, \tau, u, k$) of your choice,
        \emph{without} honest majority, but with $n - t > 0$.

        \begin{enumerate}
            \item Describe an execution where Safety is violated.
            \item Describe an execution where Liveness is violated.
            \item Is it possible to have an execution with a Safety
                violation if chain quality is \emph{not} violated?
            \item Is it possible to have an execution with a Liveness
                violation if chain quality is \emph{not} violated?
        \end{enumerate}

        For questions (a) and (b) above: What is the strategy of the adversary? What
        blocks and what transactions must the adversary produce to cause this ledger
        virtue violation? Draw a block tree and a timeline of the execution
        illustrating which round each block was mined in, which transactions are
        included in which block, whether a block was computed by an honest or an
        adversarial party, and which honest parties have adopted which chain.
        You do not need to calculate values of parameters that are not necessary
        to support the respective statement or concrete probabilities.

        \item Consider an execution with $n = 3$ parties of which $t = 1$ is
            adversarial, target $T = 2^{226}$, security parameter $\kappa = 256$ and
            hash rate $q = 1$, and $k = 6$.

            \begin{enumerate}
                \item Calculate the numeric probability of a successful round.
                \item Calculate the numeric probability of a convergence
                    opportunity.
                \item What is the numeric probability that the first 10 rounds
                    are all successful?
            \end{enumerate}

            You do not need to concretely calculate the numerical values.

        \item In the above scenario, we give the adversary the fictitious
            ability to ``snoop'' all the queries to the Random Oracle and to choose
            whatever $\kappa$-bit answer she wishes to one (honest or adversarial) fresh
            query \emph{per round}. The Random Oracle continues to use its cache to
            respond consistently. Can this adversary break common prefix? What strategy
            should she follow?

        \item In the above scenario, we give the adversary the
            fictitious ability to ``snoop'' all the queries to the Random Oracle and to
            choose whatever $\kappa$-bit answer she wishes to one (honest or
            adversarial) fresh query \emph{per execution}. The Random Oracle continues
            to use its cache to respond consistently. Can this adversary break common
            prefix? What strategy should she follow?
        \end{enumerate}

        \item Consider a \emph{typical} Bitcoin Backbone execution correctly parameterized
            under honest majority in the static proof-of-work model.

            \begin{enumerate}
            \item An honest party $P_1$ broadcasts a new transaction $\tx_1$ at round $r_1$
                and another honest party $P_2$ broadcasts a new transaction $\tx_2$ at round
                $r_2 \geq r_1$. Both transactions are included in the \emph{same} block $B$,
                which eventually becomes stable and adopted by all honest parties. You
                correctly reason that, since the two transactions were both confirmed in the
                \emph{same} block, they must have been broadcast closely together in time.

                Calculate an upper bound $d \in \mathbb{N}$ for the interval $r_2 - r_1$
                such that we can rest assured that $r_2 - r_1 \leq d$. Your bound does not
                need to be tight, but needs to hold always for typical executions. Your
                bound can be a function of all the execution parameters ($\epsilon, \lambda,
                f, p, T, n, t, k, u, \mu, \ell, s, \tau$). Prove that your bound holds.

            \item An honest party $P_1$ broadcasts a new transaction $\tx_1$ at round $r$
                and an honest party $P_2$ broadcasts a new transaction $\tx_2$ at the
                \emph{same} round $r$. However, the transactions make it on different blocks
                $B_1$ with height $h_1$ and $B_2$ with height $h_2 > h_1$ of the same chain
                that eventually becomes stable and adopted by all honest parties. You
                correctly reason that, since the two transactions were both broadcast at the
                \emph{same} time, they must be included close together on the chain.

                Calculate an upper bound $d \in \mathbb{N}$ for the distance $h_2 - h_1$
                such that we can rest assured that $h_2 - h_1 \leq d$. Your bound does not
                need to be tight, but needs to hold always for typical executions. Your
                bound can be a function of all the execution parameters ($\epsilon, \lambda,
                f, p, T, n, t, k, u, \mu, \ell, s, \tau$). Prove that your bound holds.
            \end{enumerate}
