...
place, the property of \emph{scarcity}. Scarcity is a necessary, but not sufficient,
property of money.
...
the property of \emph{ownership}. Consensus pertains to ensuring both scarcity
and ownership. 
...
\emph{prevent} adversarial behavior by making it impossible, not by detecting it
and \emph{treating} it when it emerges. These systems will be \emph{self-enforcable}.
...
of code, as long as we can recreate the virtues of scarcity and ownership,
...
traditionally codified in the form of law.  As computer scientists, our role when implementing
...
the protocol designers. A party that does not exactly follow the protocol is considered adversarial.
...
nodes that are acting on her behalf. The treatment in which the adversary is considered to be
...
and decide to lose money, just so that we can suffer, even if there is no monetary gain for her.
...
programs (in, say, Python, C++ or JavaScript) in which we have access to a random number
generator, which we assume produces fresh, uniform and completely fair randomness every time
it is called.

Our security is analyzed with a \emph{security parameter}\index{Security parameter} in
its foundations: The parameter
...
However, we will keep the $\kappa$ parameter free throughout our analysis, but we will
...
functions are easy to work with because the observe certain \emph{closure} properties.
...
something negligible keeps it negligible, and taking any power of a negligible function
keeps it negligible.
...
a relevant name. The game is also known as the \emph{challenge}.
...
$\kappa$. When we say that the adversary $\mathcal{A}$ is \emph{polynomial}, we mean that the
adversary runs in polynomial time with respect to its input. More specifically, there exists
...
giving
her\footnote{As a convention, we will use the female pronouns for the adversary, the male pronouns for the honest
...
epitome of human intellect that modern cryptography has achieved, a feat only possible through the
...
this contradicts the assumption that $\Pi^*$ was secure, completing the proof.
...
of success of $\mathcal{A}$ is negligible, completing the proof.
