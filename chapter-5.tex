...
once every $\Delta$. If transactions are spaced apart by $\Delta$ time, then no harm
can come from double spends. We can simply adopt the strategy of accepting the first
among multiple double spending transactions, and ignoring subsequent double spends.
...
These tickets are wishful thinking. How can we get them to behave this way in a permissionless
...
``close enough'' hash preimages of the image $h = 0$; that is, we want inputs $\ctr$ to the hash function
$H$, so that $H(\ctr)$ is a small enough number. \emph{How close} is defined by the value $T$, the
\glsxtrnewsymbol[description={target}]{target}{$T$}\glsadd{target}
\emph{target}\index{Target}. We write this requirement in the form of an
inequality:
...
The problem is to find the unknown value $\ctr$ so that the inequality is satisfied. Of course,
there will be multiple $\ctr$ values that satisfy this inequality. Upon solving this inequality,
...
$\ctr$ satisfies the equation is very easy: It requires just one hash invocation.
